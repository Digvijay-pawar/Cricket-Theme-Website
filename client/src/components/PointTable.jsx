...
function PointTable() {
    return (
        <div className="container mx-auto">
            <h2 className="text-4xl font-bold text-center mb-8 mt-4">IPL 2024 Point Table</h2>
            <table className="min-w-full bg-white border border-gray-300 rounded-lg shadow-md">
                <thead>
                    <tr className="bg-gray-800 text-white">
                        ...
                        <th className="py-4">Wins</th>
                        <th className="py-4">Losses</th>
                        <th className="py-4">Points</th>
                        <th className="py-4">NRR</th>
                        <th className="py-4">Recent Form</th>
                    </tr>
                </thead>
                <tbody>
                    {teamsData.map((team, index) => {
                        return (
                            ...
                            >
                                ...
                                <td className="text-left py-4 pl-4 flex items-center">
                                    <img src={team.logo} alt={`${team.name} logo`} className="h-6 w-6 mr-2" />
                                    {team.name}
                                </td>
                                <td className="text-center py-4">{team.matches}</td>
                                <td className="text-center py-4">{team.wins}</td>
                                <td className="text-center py-4">{team.losses}</td>
                                <td className="text-center py-4">{team.points}</td>
                                <td className="text-center py-4">{team.nrr.toFixed(2)}</td>
                                <td className="text-center py-4">{team.recentForm}</td>
                            </tr>
                        );
                    })}
                </tbody>
